...
title_zh: HiSpatial：在视觉语言模型中实现分层 3D 空间理解
...
tldr: 本研究提出HiSpatial框架，旨在提升视觉语言模型（VLM）的3D空间理解能力。通过将空间学习分解为从几何感知到抽象推理的四个层级，并构建包含500万张图像的大规模自动化3D VQA数据集，结合引入度量尺度点图的RGB-D VLM模型，显著增强了模型在复杂3D场景下的推理性能。实验证明该方法在多个基准测试中达到SOTA，超越了Gemini-2.5-pro等闭源模型。
selection_source: fresh_fetch
motivation: 现有的视觉语言模型在从2D图像推断3D结构及进行高层空间推理方面仍面临挑战。
method: 提出了一个四层级的层次化学习框架，并构建了大规模自动化3D VQA流水线及集成度量尺度点图的RGB-D VLM。
result: 该方法在多个空间理解基准测试中刷新了纪录，性能优于专门的空间模型及GPT-5等大型商业模型。
conclusion: 层次化的任务设计能有效促进模型涌现出更强的3D空间智能，并揭示了不同层级任务间的依赖关系。
---

## 摘要
为视觉语言模型（VLM）实现类人空间智能，需要从 2D 观测中推断 3D 结构，识别 3D 空间中的物体属性和关系，并进行高层空间推理。在本文中，我们提出了一个原则性的分层框架，将 VLM 中的 3D 空间理解学习分解为从几何感知到抽象空间推理的四个渐进复杂层级。在该框架的指导下，我们构建了一个自动化流水线，处理了约 500 万张图像和超过 4500 万个物体，为 VLM 的有监督微调生成了涵盖多种任务和场景的 3D 空间视觉问答（VQA）对。我们还开发了一种 RGB-D VLM，将米制尺度点图作为辅助输入，以进一步增强空间理解能力。大量实验表明，我们的方法在多个空间理解和推理基准测试中达到了最先进的性能，超越了专门的空间模型以及 Gemini-2.5-pro 和 GPT-5 等大型闭源系统。此外，我们的分析揭示了分层任务级别之间明确的依赖关系，为多层级任务设计如何促进 3D 空间智能的涌现提供了新的见解。
