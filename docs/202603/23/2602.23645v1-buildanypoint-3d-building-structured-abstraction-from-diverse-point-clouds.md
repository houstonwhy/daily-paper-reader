...
tldr: 本研究提出 BuildAnyPoint，一个针对多样化分布点云（如机载 LiDAR 和 SfM）的 3D 建筑结构化重建生成框架。为解决噪声和稀疏性问题，设计了松散级联扩散 Transformer (Loca-DiT)，通过潜在扩散模型恢复点云分布，并利用解码器 Transformer 自动回归生成紧凑网格。该方法在建筑抽象和点云补全任务上均表现优异，显著提升了表面精度和分布均匀性。
selection_source: carryover_cache
motivation: 针对机载 LiDAR 和 SfM 等多样化且受限的点云数据，现有的建筑结构化抽象方法难以生成高质量的艺术家级网格。
method: 提出 Loca-DiT 框架，结合条件潜在扩散模型恢复点云分布，并利用仅解码器的 Transformer 实现条件自动回归网格生成。
result: 在建筑抽象任务中取得了显著的定性和定量提升，并在点云补全基准测试中展现出更强的表面精度和均匀性。
conclusion: 该研究证明了显式 3D 生成先验在处理不确定点云输入并生成结构化建筑模型方面的有效性。
---

## 摘要
我们提出了 BuildAnyPoint，这是一个新颖的生成框架，用于从具有多样化分布的点云（例如通过机载 LiDAR 和运动恢复结构（SfM）捕获的点云）中进行结构化三维建筑重建。为了在这种高度欠约束的场景下恢复艺术家创作的建筑抽象，我们利用了显式三维生成先验在自回归网格生成中的作用。具体而言，我们设计了一个松散级联扩散 Transformer（Loca-DiT），它首先从噪声或稀疏点中恢复潜在分布，随后通过自回归方式将其封装为紧凑的网格。我们首先通过训练以输入点云为条件的潜在扩散模型，将分布恢复公式化为条件生成任务，然后针对恢复的点云定制了一个仅解码器（decoder-only）的 Transformer，用于条件自回归网格生成。与之前的建筑抽象方法相比，我们的方法在定性和定量方面都取得了显著提升。此外，我们方法的有效性通过其恢复的点云在建筑点云补全基准测试中的强劲表现得到了证明，这些点云展现出了更高的表面精度和分布均匀性。
