...
title_zh: E2EGS：面向无位姿三维重建的从事件到边缘的高斯泼溅
authors: "Yunsoo Kim, Changki Sung, Dasol Hong, Hyun Myung"
date: 2026-03-16
pdf: "https://arxiv.org/pdf/2603.14684v1"
tags: ["query:3dgen"]
score: 6.0
evidence: 使用高斯泼溅进行无位姿3D重建
tldr: 针对传统3D高斯泼溅在快速运动或恶劣光照下依赖高质量RGB和精确位姿的问题，本文提出E2EGS框架。该框架仅利用事件相机数据，通过时空相干性分析提取边缘信息，并以此引导高斯初始化、位姿跟踪和束调整，实现了无需预知位姿的3D重建，在合成和真实数据集上均表现出优异的重建质量和轨迹精度。
selection_source: fresh_fetch
motivation: 传统3D重建方法在高速运动或极端光照下因依赖RGB图像和已知位姿而受限，而现有事件相机方法仍难以摆脱对位姿或预训练深度模型的依赖。
method: 提出一种基于补丁的时间相干性分析方法来提取边缘，并利用边缘信息进行结构感知的高斯初始化及边缘加权损失优化。
result: 在合成和真实数据集上的实验表明，E2EGS在重建质量和轨迹估计精度方面均优于现有方法。
conclusion: E2EGS建立了一个完全无需位姿的事件驱动3D重建范式，证明了边缘信息在提升事件流轨迹估计和场景重建中的关键作用。
---

## 摘要
神经辐射场 (NeRF) 和 3D 高斯泼溅 (3DGS) 的出现推动了新视角合成 (NVS) 的发展。然而，这些方法需要高质量的 RGB 输入和精确的对应位姿，限制了在快速相机运动或恶劣光照等现实条件下的鲁棒性。事件相机以高时间分辨率和宽动态范围捕捉每个像素的亮度变化，能够实现对动态场景的精确感知，并提供了一个极具前景的解决方案。然而，现有的基于事件的 NVS 方法要么假设位姿已知，要么依赖于受限于初始观测的深度估计模型，在相机穿越先前未见区域时无法泛化。我们提出了 E2EGS，这是一个仅在事件流上运行的无位姿框架。我们的核心见解是，边缘信息提供了对于准确轨迹估计和高质量 NVS 至关重要的丰富结构线索。为了从噪声事件流中提取边缘，我们利用了边缘和非边缘区域截然不同的时空特征。事件相机的运动在边缘处产生一致的事件，而非边缘区域则产生稀疏噪声。我们通过基于补丁的时间相干性分析来利用这一点，该分析通过测量局部方差来提取边缘，同时鲁棒地抑制噪声。提取的边缘引导了结构感知的高斯初始化，并在初始化、跟踪和光束法平差过程中实现了边缘加权损失。在合成和真实数据集上的广泛实验表明，E2EGS 实现了卓越的重建质量和轨迹精度，为基于事件的三维重建建立了一个完全无位姿的范式。
