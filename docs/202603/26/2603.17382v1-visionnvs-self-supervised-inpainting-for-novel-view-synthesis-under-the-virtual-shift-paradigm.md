...
tldr: 针对自动驾驶新视角合成（NVS）中缺乏新轨迹真值图像的监督难题，本文提出VisionNVS框架。该框架通过“虚拟偏移”策略，利用单目深度模拟遮挡模式并映射至原图，将视角合成重构为自监督修复任务，从而直接利用原始图像作为完美监督。此外，通过伪3D接缝合成策略增强空间一致性。实验证明，该方法在几何保真度和视觉质量上优于依赖LiDAR的基线，为大规模驾驶模拟提供了鲁棒的纯相机方案。
selection_source: fresh_fetch
motivation: 解决自动驾驶场景下新视角合成因缺乏训练阶段真值图像而导致的监督差距和域偏移问题。
method: 引入虚拟偏移范式将视角合成重构为自监督修复任务，并结合伪3D接缝合成策略来建模相机间的测光差异和标定误差。
result: 实验结果显示，VisionNVS在几何准确性和图像质量上均显著优于现有的依赖LiDAR的先进基线方法。
conclusion: 该研究证明了通过自监督修复范式可以实现高质量、高一致性的纯相机视角合成，为可扩展的驾驶模拟提供了有效的解决方案。
---

## 摘要
自动驾驶中新视角合成（NVS）的一个根本瓶颈是新轨迹上固有的监督差距：模型在推理期间被要求合成未见过的视角，但在训练期间缺乏这些偏移位姿的真值图像。在本文中，我们提出了 VisionNVS，这是一个仅依靠摄像头的框架，它从根本上将视角合成从一个病态的外推问题重新定义为一个自监督的修复任务。通过引入“虚拟偏移”（Virtual-Shift）策略，我们利用单目深度代理来模拟遮挡模式，并将其映射到原始视图上。这种范式转变允许使用原始录制的图像作为像素级的完美监督，有效地消除了以往方法中固有的领域差距。此外，我们通过伪 3D 接缝合成（Pseudo-3D Seam Synthesis）策略解决了空间一致性问题，该策略在训练期间整合了相邻摄像头的视觉数据，以显式建模现实世界的光度差异和校准误差。实验表明，与依赖 LiDAR 的基准模型相比，VisionNVS 实现了更优的几何保真度和视觉质量，为可扩展的驾驶模拟提供了一个鲁棒的解决方案。
