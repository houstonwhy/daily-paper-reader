...
title_zh: CustomTex：通过多参考定制实现高保真室内场景纹理化
...
## 摘要
创建高保真、可定制的 3D 室内场景纹理仍然是一个重大挑战。虽然文本驱动的方法提供了灵活性，但它们缺乏细粒度、实例级控制的精度，且通常生成的纹理质量不足，存在伪影和烘焙阴影。为了克服这些局限性，我们提出了 CustomTex，这是一个由参考图像驱动的实例级、高保真场景纹理化新框架。CustomTex 输入一个未贴图的 3D 场景和一组指定每个物体实例所需外观的参考图像，并生成一个统一的高分辨率纹理贴图。我们方法的核心是一种双重蒸馏方法，它将语义控制与像素级增强分离开来。我们采用了配备实例交叉注意力的语义级蒸馏，以确保语义合理性和“参考-实例”对齐，并采用像素级蒸馏来增强视觉保真度。两者都统一在变分分数蒸馏 (VSD) 优化框架内。实验表明，与现有最先进的方法相比，CustomTex 实现了与参考图像精确的实例级一致性，并生成了具有更高清晰度、更少伪影和极少烘焙阴影的纹理。我们的工作为高质量、可定制的 3D 场景外观编辑建立了一条更直接、更用户友好的路径。
