...
## 摘要
最近的研究表明，神经网络可以在没有显式 3D 重建的情况下执行诸如新视角合成 (NVS) 等 3D 任务。尽管如此，我们认为强大的 3D 归纳偏置在设计此类网络时仍然是有益的。我们通过引入 LagerNVS 来证明这一点，这是一种基于“3D 感知”潜特征的用于 NVS 的编码器-解码器神经网络。该编码器初始化自一个使用显式 3D 监督预训练的 3D 重建网络。它与一个轻量级解码器配对，并使用光度损失进行端到端训练。LagerNVS 实现了最先进的确定性前馈新视角合成（包括在 Re10k 数据集上达到 31.4 PSNR），无论是否已知相机参数，均可实时渲染，能够泛化到野外数据，并可与扩散解码器配对进行生成式外推。
