...
title_zh: ReLi3D：基于解耦光照的可重光照多视图三维重建
...
method: 采用 Transformer 交叉调节架构融合多视图输入，并结合双路径预测策略与可微分蒙特卡洛多重重要性采样渲染器进行端到端训练。
result: 实现了在不到一秒的时间内从稀疏视图中生成具有精确几何、材质和光照信息的完整可重光照 3D 资产。
conclusion: 该研究通过统一重建任务并引入多视图约束，显著提升了光照解耦效果，为实时生成高质量 3D 资产提供了高效方案。
---

## 摘要
从图像中重建三维资产长期以来需要独立的几何重建、材质估计和光照恢复流水线，每种流水线都有其局限性和计算开销。我们提出了 ReLi3D，这是第一个统一的端到端流水线，能够在不到一秒的时间内，从稀疏多视图图像中同时重建完整的三维几何、空间变化的基于物理的材质以及环境光照。我们的核心见解是，多视图约束可以显著改善材质与光照的解耦，而这一问题对于单图像方法而言在根本上是病态的。我们方法的关键在于通过 Transformer 交叉调节架构融合多视图输入，随后采用一种新颖的统一双路径预测策略。第一条路径预测物体的结构和外观，而第二条路径从图像背景或物体反射中预测环境光照。这与可微分蒙特卡罗多重重要性采样渲染器相结合，创建了一个最优的光照解耦训练流水线。此外，通过我们的混合域训练协议（结合了合成 PBR 数据集与真实世界的 RGB 捕获），我们在几何、材质准确性和光照质量方面建立了具有泛化性的结果。通过将先前独立的重建任务统一到单个前向传递中，我们实现了完整、可重光照三维资产的近乎瞬时生成。项目页面：https://reli3d.jdihlmann.com/
