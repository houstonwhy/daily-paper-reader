...
score: 6.0
evidence: 3D重建表示
tldr: Few TensoRF 结合了 TensoRF 的高效张量表示与 FreeNeRF 的频率驱动正则化技术，旨在解决少样本视角下的 3D 重建难题。通过引入频率掩码和遮挡掩码，该方法在保持 10-15 分钟快速训练优势的同时，显著提升了稀疏输入下的重建质量和稳定性。在 Synthesis NeRF 和 THuman 2.0 数据集上的实验证明，其在 PSNR 指标上大幅优于原始 TensoRF，是兼顾效率与效果的少样本 3D 重建方案。
selection_source: fresh_fetch
motivation: 针对 TensoRF 在少样本输入下重建质量不稳定且易产生伪影的问题，寻求一种高效且稳健的正则化改进方案。
method: 将 TensoRF 的张量分解表示与 FreeNeRF 的频率掩码及遮挡掩码技术相结合，以优化稀疏视角下的辐射场学习。
