...
title_zh: 可观测性差距：为什么输出层级的人类反馈对大语言模型编程智能体失效
...
tldr: "本研究探讨了LLM编程智能体在“获得性自主”设定下的表现，即智能体通过人类对视觉输出的反馈从零构建函数库。在Blender 3D场景生成任务中，尽管智能体能重构基础功能，但在仅依赖输出层反馈时，全场景成功率为0%。研究揭示了“观测性鸿沟”：代码逻辑与视觉输出间的复杂映射导致反馈无法定位根因，引发失败模式震荡。通过引入代码级知识干预可恢复收敛，强调了中间观测性在人机协作中的重要性。"
selection_source: fresh_fetch
motivation: 探究仅依赖输出层的人类反馈是否足以支持LLM编程智能体通过增量学习构建复杂的函数库。
method: 在Blender 3D场景生成任务中，让智能体从零开始，仅根据人类对生成图像的反馈来迭代优化其编程函数库。
result: "实验发现仅靠输出反馈会导致0%的场景成功率，并引发失败模式的持续震荡，而非性能收敛。"
conclusion: 研究提出了“观测性鸿沟”概念，强调在具有深层因果链的任务中，有效的智能体协作需要超越输出层的中间过程观测性。
---

## 摘要
大语言模型（LLM）多智能体编程系统通常在设计时固定智能体的能力。我们研究了一种替代设定，即“获得性自主”（earned autonomy），在这种设定下，编程智能体从零预定义函数开始，仅通过对视觉输出的轻量级人类反馈，逐步构建可重用的函数库。我们在基于 Blender 的 3D 场景生成任务中评估了这一设定，该任务需要空间推理和程序化几何控制。尽管智能体重新发现了与人类参考实现相当的核心实用函数，但在多种指令粒度下，仅凭输出反馈，其全场景成功率均为 0%，而成功需要同时满足物体完整性、地面接触、碰撞避免和比例合理性。我们的分析确定了一个结构性的可观测性差距：错误源于代码逻辑和执行状态，而人类评估仅发生在输出层，且内部状态到可见结果的多对一映射阻止了症状级反馈可靠地识别根本原因。这种不匹配导致了持续的失败模式振荡而非收敛。一项注入极少量代码级知识的诊断性干预恢复了收敛，有力地支持了这一解释，即主要瓶颈在于反馈的可观测性而非编程能力。我们将这一现象形式化为在内部代码逻辑与感知结果之间存在深层因果链领域中的“反馈悖论”，并认为在这些设定中，有效的人机协作需要超越仅输出评估的中间可观测性。
