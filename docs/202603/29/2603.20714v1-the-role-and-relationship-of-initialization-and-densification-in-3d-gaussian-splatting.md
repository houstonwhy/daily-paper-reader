...
tldr: 本研究系统探讨了 3D 高斯泼溅（3DGS）中初始化点云与加密阶段之间的关系。通过构建全新的基准测试，对比了激光扫描、多视图立体、单目深度及稀疏 SfM 等多种初始化方式。研究发现，现有的加密方案难以充分利用高密度初始化的优势，其最终重建质量往往无法显著优于传统的稀疏 SfM 初始化，揭示了当前 3DGS 优化机制的局限性。
selection_source: fresh_fetch
motivation: 旨在探究 3DGS 中初始化点云的密度与质量如何影响加密过程及最终的场景重建效果。
method: 提出了一个包含多种初始化类型（如激光扫描、MVS、单目深度）与不同加密方案组合的新型基准测试。
result: 实验证明当前的加密方法无法有效利用密集初始化，其表现通常与基于稀疏 SfM 的初始化相当。
conclusion: 现有的 3DGS 加密策略在处理高质量初始数据时存在瓶颈，未能实现预期的性能提升。
---
