...
motivation: 现有的 3D 重建方法难以准确建模复杂外观，而扩散生成模型虽能合成逼真图像却缺乏精确的视角控制。
method: 提出 GO-Renderer 框架，通过 3D 重建代理引导视频扩散模型，在任意视角和光照条件下实现高质量物体渲染。
result: 实验表明 GO-Renderer 在新视角合成、新光照环境渲染以及物体视频插入等任务上均取得了领先的性能。
conclusion: 该研究成功将 3D 几何控制与生成式扩散模型相结合，为实现高效且逼真的物体渲染提供了新路径。
---
