...
## 摘要
3D 高斯泼溅（3D Gaussian Splatting）在静态场景的新视角合成中展现了卓越的实时渲染能力和优异的视觉质量。基于这些优势，研究人员逐渐将 3D 高斯扩展到动态场景重建。在各种技术中，基于变形场的方法已成为一种极具前景的方法。这些方法在标准场中维护 3D 高斯属性，并利用变形场在时间序列中对该场进行变换。然而，这些方法经常面临渲染速度不理想、对初始点云的高度依赖以及在昏暗场景中容易陷入局部最优等挑战。为了克服这些局限性，我们提出了 FRoG，一个用于高质量动态场景重建的高效且鲁棒的框架。FRoG 将逐高斯嵌入与由粗到细的时间嵌入策略相结合，通过时间嵌入的早期融合加速渲染。此外，为了增强对稀疏初始化的鲁棒性，我们引入了一种新型的深度和误差引导采样策略。该策略在低偏差的初始位置为标准场填充新的 3D 高斯，显著减轻了变形场的优化负担，并改善了静态和动态区域的细节重建。此外，通过调节不透明度变化，我们缓解了昏暗场景中的局部最优问题，提高了颜色保真度。综合实验结果验证了我们的方法在保持最先进视觉质量的同时，实现了更快的渲染速度。
