...
title_zh: RefracGS：利用 3D 高斯光线追踪实现穿过折射水面的新视角合成
...
## 摘要
穿过非平面折射表面的新视角合成（NVS）由于严重的、随空间变化的光学畸变而面临根本性的挑战。虽然像 NeRF 和 3D 高斯泼溅（3DGS）这样的最新表示方法在 NVS 方面表现出色，但它们关于光线直线传播的假设在这些条件下失效，从而导致明显的伪影。为了克服这一局限性，我们提出了 RefracGS，这是一个联合重建折射水面和界面下方场景的框架。我们的核心见解是将折射边界与目标物体显式解耦：折射表面通过神经高度场建模以捕捉波浪几何形状，而底层场景则表示为 3D 高斯场。我们制定了一种折射感知的高斯光线追踪方法，该方法利用斯涅尔定律（Snell's law）精确计算非线性光线轨迹，并高效渲染底层高斯场，同时将损失梯度反向传播到参数化的折射表面。通过对这两种表示进行端到端的联合优化，我们的方法确保了高保真度的 NVS 和视角一致的表面恢复。在具有复杂波浪的合成和真实场景中的实验表明，RefracGS 在视觉质量上优于先前的折射方法，同时实现了快 15 倍的训练速度和 200 FPS 的实时渲染。RefracGS 的项目页面位于 https://yimgshao.github.io/refracgs/。
