...
title_zh: 结合自约束先验的高保真表面重建3D高斯泼溅
...
## 摘要
3D表面渲染在辐射场建模领域（通过3DGS或NeRF）经历了革命性的变化。尽管3DGS在渲染质量和速度上表现出优于NeRF的优势，但在利用3DGS恢复高保真表面方面仍存在提升空间。为解决这一问题，我们提出了一种自约束先验来约束3D高斯的学习，旨在实现更精确的深度渲染。我们的自约束先验源于通过融合当前3D高斯渲染的深度图所获得的TSDF网格。该先验测量估计表面周围的距离场，提供一个以表面为中心的带状区域，从而对3D高斯施加更具体的约束，例如移除带外高斯、将高斯移近表面，以及以几何感知的方式调节不透明度。更重要的是，我们的先验可以利用通常更准确、更完整的最新深度图像进行定期更新。此外，该先验还可以逐步收窄带状区域以强化约束。我们验证了这一思路，并在广泛使用的基准测试评估中证明了其相较于现有最先进方法的优越性。
