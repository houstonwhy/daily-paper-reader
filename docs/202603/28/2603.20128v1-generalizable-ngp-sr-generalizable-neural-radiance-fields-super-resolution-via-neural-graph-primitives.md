...
## 摘要
神经辐射场 (NeRF) 实现了逼真的新视图合成，但在需要高分辨率 (HR) 渲染时成本高昂，因为高分辨率输出需要密集的采样和更高容量的模型。此外，在 2D 空间中对每个视图的渲染结果进行简单的超分辨率处理往往会破坏多视图一致性。我们提出了 Generalizable NGP-SR，这是一种 3D 感知的超分辨率框架，可直接从低分辨率 (LR) 的带位姿图像中重建高分辨率辐射场。NGP-SR 基于神经图形基元 (NGP) 构建，将辐射预测以 3D 坐标和学习到的局部纹理标记 (tokens) 为条件，从而能够恢复辐射场内的高频细节，并在无需外部高分辨率参考或事后 2D 上采样的情况下，生成视图一致的高分辨率新视图。重要的是，我们的模型具有可泛化性：一旦训练完成，它就可以应用于未见场景，并从新视角进行渲染，而无需逐场景优化。在多个数据集上的实验表明，与以往基于 NeRF 的超分辨率方法相比，NGP-SR 在重建质量和运行效率方面均有一致的提升，为可扩展的高分辨率新视图合成提供了一种实用的解决方案。
