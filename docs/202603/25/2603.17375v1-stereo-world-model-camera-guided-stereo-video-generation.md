...
motivation: 现有的单目转立体方案效率低下且难以保证几何一致性，亟需一种能直接在RGB空间学习双目几何的端到端生成模型。
method: 引入统一相机帧RoPE进行视角一致性调节，并利用立体感知注意力分解（3D内视图+水平行注意力）来降低计算开销并捕捉视差对应。
result: "相比传统流水线，生成速度提升3倍以上，视点一致性提高5%，且在立体一致性和视差准确度上表现更优。"
conclusion: StereoWorld 证明了通过端到端学习和几何先验引导，可以高效实现高质量、符合物理规律的立体视频生成。
---
