...
## 摘要
尽管动态场景的新视角合成（NVS）已取得显著进展，但重建时间一致的几何表面仍然是一个挑战。神经辐射场（NeRF）和 3D 高斯泼溅（3DGS）提供了强大的动态场景渲染能力；然而，仅依赖光度优化往往会导致几何二义性。这会导致表面不连续、严重的伪影以及随时间推移出现的表面破损。为了解决这些局限性，我们提出了 DySurface，这是一个新颖的框架，它在动态场景中桥接了显式高斯的有效性与隐式有符号距离函数（SDF）的几何保真度。我们的方法解决了 3DGS 的前向变形（标准空间 $\rightarrow$ 动态空间）与体素 SDF 渲染所需的后向变形（动态空间 $\rightarrow$ 标准空间）之间的结构差异。具体而言，我们提出了 VoxGS-DSDF 分支，利用变形后的高斯构建动态稀疏体素网格，为隐式 SDF 场提供显式的几何引导。这种显式锚定有效地正则化了体渲染过程，显著提高了表面重建质量，具有水密边界和细节表现。定量和定性实验表明，DySurface 在几何精度指标上显著优于最先进的基准方法，同时保持了具有竞争力的渲染性能。
