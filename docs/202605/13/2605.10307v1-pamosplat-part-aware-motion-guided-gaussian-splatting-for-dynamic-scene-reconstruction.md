...
tags: ["query:3dgen"]
score: 7.0
evidence: 使用高斯泼溅进行动态场景重建
tldr: 针对动态场景重建中复杂运动建模难的问题，本文提出PaMoSplat框架。该方法结合了部件感知与运动先验，通过3D图聚类初始化高斯部件，并利用光流引导的差分进化算法估计部件刚性运动。PaMoSplat在渲染质量、跟踪精度和收敛速度上均优于现有方法，并支持4D场景编辑等下游任务。
selection_source: fresh_fetch
motivation: 现有的动态3D高斯泼溅方法在处理大幅度、复杂运动时，难以兼顾高保真渲染与精确跟踪。
method: 通过将多视图分割掩码提升至3D空间进行部件初始化，并利用光流引导的差分进化算法为部件运动提供鲁棒的预热优化。
result: 在多个真实场景评估中，该方法在渲染质量、跟踪精度和收敛速度方面均表现出显著优势。
conclusion: PaMoSplat通过引入部件级先验和运动引导，有效提升了动态场景的重建性能，并扩展了4D场景编辑的应用潜力。
---
