...
score: 6.0
evidence: 3D重建与几何聚合
tldr: 针对流式3D重建中外观驱动内存更新导致的冗余和几何不稳定问题，本文提出一种射线感知指针内存机制。该方法通过统一建模空间位置和视角方向，并采用自适应的“保留或替换”更新策略，实现了高效的几何聚合与长效内存管理。结合位姿优化，该框架在保持推理效率的同时，显著提升了重建的稳定性和位姿精度。
selection_source: fresh_fetch
motivation: 传统的基于外观相似性的内存更新方式容易导致观测冗余和视角变化时的几何不稳定。
method: 提出射线感知指针内存，结合空间位置与视角方向进行联合推理，并采用自适应更新策略替代传统的融合压缩。
result: 实验证明该设计显著增强了长期重建的稳定性，提高了相机位姿精度，并支持高效的流式推理。
conclusion: 该研究为可扩展且抗漂移的在线3D重建提供了一个基于射线感知内存的原则性框架。
---

## 摘要
从连续图像流进行稠密三维重建既需要准确的几何聚合，也需要稳定的长期内存管理。最近的前馈重建框架通过持久化内存表示来整合观测结果，但大多数在更新内存时主要依赖于基于外观的相似性。这种外观驱动的整合在视点发生变化时，往往会导致观测结果的冗余累积和几何结构的不稳定。在这项工作中，我们为流式三维重建提出了一种射线感知指针内存，它在统一的内存表示中显式地建模了空间位置和视角方向。每个内存指针存储其三维位置、相关的射线方向和特征嵌入，使系统能够共同推理几何邻近性和视点一致性。基于这种表示，我们引入了一种自适应指针更新策略，用“保留或替换”机制取代了传统的基于融合的内存压缩。系统不再对附近的观测结果进行平均，而是选择性地保留信息丰富的指针，同时丢弃冗余指针，在保持有限内存增长的同时保留独特的几何结构。此外，对空间距离和射线方向差异的联合推理使系统能够以统一的方式区分局部冗余、新观测和潜在的回环访问。当检测到回环候选时，会触发位姿精细化，以增强整个重建过程中的全局几何一致性。大量实验表明，所提出的射线感知内存设计在保持高效流式推理的同时，显著提高了长期重建的稳定性和相机位姿精度。我们的方法为从图像流进行可扩展且抗漂移的在线三维重建提供了一个原则性的框架。
