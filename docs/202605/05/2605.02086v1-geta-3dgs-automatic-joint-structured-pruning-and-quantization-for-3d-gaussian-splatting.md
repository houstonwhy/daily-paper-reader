...
tldr: 针对3D高斯泼溅（3DGS）模型体积庞大、难以在移动端部署的问题，本文提出了GETA-3DGS，这是首个端到端自动联合结构化剪枝与量化的框架。该方法通过构建感知量化的依赖图、引入渲染感知的显著性评分以及异构混合精度方案，实现了在无需手动调整阈值的情况下，对3DGS属性进行协同优化。实验表明，该方法在保持高质量渲染的同时，能将存储空间减少约5倍，且可与现有熵编码技术无缝结合。
selection_source: fresh_fetch
motivation: 现有的3DGS压缩方法通常将剪枝与量化视为独立阶段，且依赖手动调节的启发式阈值，限制了跨场景的泛化能力和对存储预算的精确控制。
method: 提出了一种端到端自动优化框架，结合了感知量化的依赖图、融合渲染贡献的显著性评分以及基于投影偏显著性引导下降的异构混合精度分配方案。
result: 在Mip-NeRF 360等多个数据集上，该方法在无需场景特定阈值的情况下，实现了比原始3DGS约5倍的存储缩减，且渲染质量显著优于统一位宽量化方案。
conclusion: GETA-3DGS证明了联合结构化剪枝与异构量化在3DGS压缩中的高效性，为实现自动化、高压缩比的3D场景表示提供了有力工具。
---

## 摘要
3D 高斯泼溅 (3DGS) 是一种用于实时照片级真实感新视角合成的前沿表示方法，然而单个高保真场景通常占用数百兆字节到数千兆字节，超出了移动、沉浸式和体积视频平台的预算。现有的 3DGS 压缩方法（如 HAC++、FlexGaussian、LP-3DGS）将剪枝、量化和熵编码视为独立阶段，并依赖于手动调整的启发式方法（如不透明度阈值、固定位宽、SH 截断），这限制了跨场景的泛化能力，并阻碍了用户指定目标比特率或质量预算。我们提出了 GETA-3DGS，据我们所知，这是首个针对 3DGS 的端到端自动联合结构化剪枝与量化框架。基于用于深度网络联合剪枝-量化的 GETA，我们的贡献包括：(i) 一种感知 3DGS 的量化感知依赖图 (QADG)，将每个高斯基元视为一个包含五个属性子节点和感知阶数的 SH 子节点的组；(ii) 一种渲染感知显著性，将透射率加权贡献、屏幕空间梯度和像素覆盖范围融合为高斯级的显著性得分；以及 (iii) 一种异构的逐属性混合精度方案，在投影部分显著性引导 (PPSG) 下降保证下与结构稀疏性共同优化。在 Mip-NeRF 360、Tanks and Temples 和 Deep Blending 数据集上，GETA-3DGS 直接作用于原始高斯基元而非事后锚点表示，在无需逐场景阈值的情况下，相比原始 3DGS 实现了约 5 倍的存储缩减。位宽策略是主要的率失真杠杆：与我们的异构分配相比，在视角相关场景中，统一的 6 位限制会导致高达 -6.74 dB 的损失，这与我们开发的信息论反向注水分析相吻合。GETA-3DGS 与现有编解码器互补：熵编码（如 HAC++、CompGS）处于下游，因此两者可以组合使用。
