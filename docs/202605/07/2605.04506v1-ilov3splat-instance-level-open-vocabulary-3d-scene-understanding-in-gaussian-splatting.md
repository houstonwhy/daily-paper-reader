...
tldr: Ilov3Splat是一个基于3D高斯泼溅（3D-GS）的实例级开放词汇3D场景理解框架。针对现有方法在跨视图一致性和实例推理方面的不足，该方法通过多分辨率哈希编码CLIP特征和基于SAM掩码的对比学习，构建了视图一致的语义与实例特征场。它支持通过自然语言查询识别任意3D对象，无需类别监督，在物体选择和实例分割任务上表现优异。
selection_source: fresh_fetch
motivation: 现有3D场景理解方法在跨视图一致性和实例级推理方面存在局限，导致下游任务精度不足。
method: 通过多分辨率哈希嵌入CLIP特征并结合SAM掩码进行对比学习，构建增强的特征场，并利用两阶段3D聚类实现实例检索。
result: 在标准基准测试中，该方法在物体选择和实例分割任务上均超越了现有的开放词汇3D-GS方法。
conclusion: Ilov3Splat为语言驱动的3D场景理解提供了一种灵活且精确的解决方案，实现了无需标注的任意物体识别。
---
