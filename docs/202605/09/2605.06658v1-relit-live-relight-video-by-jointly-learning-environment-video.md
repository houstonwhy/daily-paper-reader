...
title_zh: "Relit-LiVE: 通过联合学习环境视频进行视频重照明"
...
tags: ["query:3dgen"]
score: 6.0
evidence: 内在场景表示与神经渲染
tldr: 针对现有视频重光照方法依赖不可靠的内在分解且易产生伪影的问题，本文提出Relit-LiVE框架。该方法通过引入原始参考图像并联合预测重光照视频与逐帧环境贴图，在无需相机位姿先验的情况下，实现了物理一致且时间稳定的视频重光照。该框架支持动态光照和相机运动，在合成与真实场景中均优于现有SOTA方法，并可扩展至材质编辑、物体插入等多种下游任务。
selection_source: fresh_fetch
motivation: 现有的基于内在分解的视频重光照方法在处理真实视频时极易出现外观失真和时间不一致等问题。
method: 提出一种联合预测机制，在扩散过程中同时生成重光照视频和对齐的环境视频，并引入原始参考图像以保留场景细节。
result: 实验证明该方法在物理一致性和时间稳定性上显著超越了现有的视频重光照和神经渲染技术。
conclusion: Relit-LiVE为无需相机位姿的视频重光照提供了高效且高质量的解决方案，具有广泛的应用潜力。
---

## 摘要
最近的进展表明，大规模视频扩散模型可以通过先将视频分解为内在场景表示，然后在新的光照下进行前向渲染，从而被重新用作神经渲染器。尽管这一范式前景广阔，但它从根本上依赖于准确的内在分解，而这对于真实世界视频来说仍然高度不可靠，且在重照明过程中往往会导致外观扭曲、材质损坏以及累积的时间伪影。在这项工作中，我们提出了 Relit-LiVE，这是一个全新的视频重照明框架，它在不需要相机位姿先验知识的情况下，能够产生物理一致且时间稳定的结果。我们的核心见解是在渲染过程中显式引入原始参考图像，使模型能够恢复在内在表示中不可避免丢失或损坏的关键场景线索。此外，我们提出了一种新颖的环境视频预测公式，在单个扩散过程中同时生成重照明视频和与每个相机视角对齐的逐帧环境贴图。这种联合预测增强了强几何-光照对齐，并自然地支持动态光照和相机运动，在显著提高视频重照明物理一致性的同时，降低了对已知逐帧相机位姿的要求。大量实验表明，Relit-LiVE 在合成和真实世界基准测试中始终优于最先进的视频重照明和神经渲染方法。除了重照明，我们的框架还自然地支持广泛的下游应用，包括场景级渲染、材质编辑、物体插入和流式视频重照明。项目地址见 https://github.com/zhuxing0/Relit-LiVE。
