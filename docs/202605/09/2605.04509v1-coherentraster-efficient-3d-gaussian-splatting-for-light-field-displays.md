...
tags: ["query:3dgen"]
score: 7.0
evidence: 用于场景渲染的3D高斯泼溅
tldr: 针对光场显示器（LFD）多视图渲染计算开销大的问题，本文提出CoherentRaster框架。该方法基于3D高斯泼溅（3DGS），通过跨视图相干属性重用减少冗余计算，并利用视图相干重映射解决交错子像素布局导致的GPU效率低下问题。该框架在消费级硬件上实现了实时、高质量的光场合成，显著提升了渲染效率。
selection_source: fresh_fetch
motivation: 传统3DGS在光场显示器上直接应用时，由于多视图需求和交错子像素布局，面临巨大的计算开销和GPU内存访问效率低下的挑战。
method: 提出一种基于子像素级光栅化的框架，结合跨视图相干属性重用和视图相干重映射技术，优化冗余计算并恢复内存访问效率。
result: 在消费级硬件上实现了高效的实时光场渲染，同时保持了高图像质量。
conclusion: CoherentRaster为光场显示提供了一种高效的3DGS渲染方案，克服了空间不相干布局带来的性能瓶颈。
---
