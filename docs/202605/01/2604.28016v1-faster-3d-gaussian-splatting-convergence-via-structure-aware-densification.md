...
tldr: 针对3D高斯泼溅（3DGS）在密度控制中无法区分几何错位与频率混叠的问题，本文提出一种结构感知加密框架。通过多尺度频率分析评估局部纹理细节，并引入各向异性分裂策略，根据频率违规指标η精确增加高斯基元。该方法显著加快了收敛速度，并在保持高效渲染的同时提升了高频区域的重建质量。
selection_source: fresh_fetch
motivation: 标准3DGS依赖屏幕空间梯度进行加密，容易导致高频纹理模糊或无效的过度加密。
method: 结合结构张量与拉普拉斯尺度空间进行多尺度频率分析，并根据各向异性频率违规指标进行针对性分裂。
result: 在标准基准测试中实现了更快的收敛速度，并显著提升了复杂纹理区域的重建精度。
conclusion: 结构感知的各向异性加密策略能有效优化3DGS的基元分布，平衡了训练效率与渲染质量。
---
