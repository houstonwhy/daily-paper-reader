...
tldr: 针对野外场景中 3D 高斯泼溅（3DGS）面临的瞬态遮挡和光照不一致挑战，本文提出 HarmoGS 框架。该方法通过语义一致性引导掩码抑制不可靠监督，并引入冲突感知梯度协调策略，通过旋转视图梯度减少方向干扰。此外，还结合了冲突感知的加密与剪枝机制，有效提升了复杂环境下的重建质量和渲染稳定性。
selection_source: fresh_fetch
motivation: 现有的图像级掩码方法无法完全消除残余遮挡或解决光照引起的多视图梯度冲突，导致 3DGS 优化不稳定并产生伪影。
method: 提出语义一致性引导掩码以精细化监督，并采用双视图梯度协调策略及冲突感知的高斯加密与剪枝来稳定优化过程。
result: 在标准野外场景基准测试中，该方法在处理复杂瞬态干扰和跨视图不一致性方面达到了最先进的渲染质量。
conclusion: 通过在图像空间监督和梯度级优化两方面解决冲突问题，HarmoGS 显著增强了 3DGS 在真实野外环境下的鲁棒性。
---

## 摘要
野外环境下的 3D 高斯泼溅（3D Gaussian Splatting）由于瞬态干扰物和光照引起的跨视图外观不一致性，仍然具有挑战性。现有方法主要依赖图像级掩码来抑制不可靠的监督，但仅靠掩码无法完全消除残留遮挡或解决光照引起的不一致性，这两者都会引入冲突的跨视图梯度。这些未解决的冲突可能会使高斯优化过程不稳定，并导致明显的重建伪影。我们提出了一个冲突感知的 3DGS 框架，从图像空间监督和梯度级优化两个方面解决这一问题。语义一致性引导的掩码学习像素级一致性分数，以自适应地细化先验掩码，并在梯度形成前抑制不可靠的监督。双视图冲突感知梯度协调策略通过将特定视图的梯度相互旋转到正交配置，进一步协调这些梯度，从而减少跨视图的负向方向干扰。我们还引入了冲突感知的加密和剪枝，以稳定高斯的增长并移除持续冲突的原语。在标准野外基准测试上的大量实验表明，我们的方法在复杂的瞬态干扰和跨视图不一致情况下，实现了最先进的渲染质量。
