...
tldr: One2Scene 针对单图生成可探索 3D 场景中存在的几何畸变和噪声问题，提出了一种将任务分解为全景图生成、3D 几何支架构建和新视角生成的框架。通过将全景图投影为多视角锚点并利用高斯泼溅（Gaussian Splatting）构建几何一致的支架，该方法在大幅度相机运动下仍能保持稳定性，实现了高质量、沉浸式的 3D 场景探索。
selection_source: fresh_fetch
motivation: 现有单图 3D 场景生成方法在视角偏离原始位置时，容易产生严重的几何畸变和伪影，难以支持自由探索。
method: 该框架首先生成全景锚点图，随后通过前馈高斯泼溅网络将其转化为 3D 几何支架，最后利用该支架引导生成任意视角下的逼真图像。
result: 实验表明，One2Scene 在全景深度估计、前馈 360° 重建和可探索 3D 场景生成方面均显著优于现有最先进方法。
conclusion: One2Scene 通过引入显式的 3D 一致性几何支架，有效解决了单图生成 3D 场景中的几何稳定性问题，支持大范围的沉浸式场景探索。
---
