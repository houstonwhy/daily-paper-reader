...
motivation: 在商业游戏引擎中自动生成涉及资产、场景和复杂逻辑的3D游戏仍然是一个极具挑战性的任务。
method: 提出AutoUE多智能体系统，利用检索增强生成减少幻觉，并结合游戏设计模式与自动化测试流水线确保代码质量。
result: 实验和新构建的数据集验证了AutoUE端到端生成3D游戏的能力，并证明了其在减少工具使用错误和提升系统鲁棒性方面的有效性。
conclusion: AutoUE通过多智能体协作和知识增强机制，为虚幻引擎环境下的自动化游戏开发和评估奠定了基础。
---
