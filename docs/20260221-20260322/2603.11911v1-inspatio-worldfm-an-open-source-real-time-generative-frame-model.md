...
tldr: InSpatio-WorldFM 是一个开源的实时生成式帧模型，旨在解决视频世界模型在高延迟和空间一致性方面的不足。它采用独立生成每一帧的范式，通过 3D 锚点和隐式空间记忆确保多视图一致性。该模型通过三阶段训练流程（预训练、可控化、蒸馏）实现，支持在消费级 GPU 上进行低延迟的实时空间推理和交互式探索，为空间智能提供了高效的生成方案。
selection_source: fresh_fetch
motivation: 传统视频世界模型依赖序列生成导致高延迟，难以满足实时空间推理和交互式模拟的需求。
method: 采用基于帧的生成范式，结合显式 3D 锚点与隐式空间记忆，并通过三阶段蒸馏流程将图像扩散模型转化为实时生成器。
result: 该模型在保持强多视图一致性的同时，实现了极低的推理延迟，支持在消费级 GPU 上进行流畅的实时交互。
conclusion: InSpatio-WorldFM 为实时世界模拟提供了一种高效且开源的替代方案，成功平衡了生成质量与运行效率。
---
