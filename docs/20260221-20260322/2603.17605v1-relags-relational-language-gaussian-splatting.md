...
motivation: 现有的3D感知方法大多以物体为中心，且在处理物体间复杂关系时往往需要高昂的特定场景训练成本。
method: 提出一种分层语言蒸馏高斯场景框架，结合高斯剪枝、多视图特征对齐以及基于图神经网络的开放词汇3D场景图构建。
result: 在开放词汇分割、场景图生成和关系引导检索等多个任务上验证了该方法的有效性、效率和可扩展性。
conclusion: ReLaGS通过联合建模分层语义和物体间关系，为实现高效且可扩展的统一3D场景理解与推理提供了新方案。
---

## 摘要
在分割、检索和关系理解等任务中实现统一的 3D 感知和推理仍然具有挑战性，因为现有方法要么是以物体为中心的，要么依赖于昂贵的物体间推理训练。我们提出了一个新颖的框架，在无需针对特定场景进行训练的情况下，构建分层语言蒸馏高斯场景及其 3D 语义场景图。高斯剪枝机制优化了场景几何，而鲁棒的多视图语言对齐策略将噪声 2D 特征聚合为准确的 3D 物体嵌入。在此层级结构之上，我们利用视觉语言衍生的标注和基于图神经网络的关系推理，构建了一个开放词汇的 3D 场景图。我们的方法通过对分层语义以及物体间/物体内关系进行联合建模，实现了高效且可扩展的开放词汇 3D 推理，并在包括开放词汇分割、场景图生成和关系引导检索在内的任务中得到了验证。项目页面：https://dfki-av.github.io/ReLaGS/
