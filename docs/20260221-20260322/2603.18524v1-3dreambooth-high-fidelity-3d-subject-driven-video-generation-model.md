...
tags: ["query:3dgen"]
score: 8.0
evidence: 3D主体驱动的视频生成与空间先验
tldr: 针对现有视频生成模型在定制化物体时缺乏3D几何一致性的问题，本文提出3DreamBooth框架。该框架通过1帧优化范式将空间几何与时间运动解耦，将稳健的3D先验植入模型。同时引入3Dapter视觉调节模块，通过非对称调节策略提取多视角几何线索，实现了高保真、视角一致的3D感知视频生成，有效解决了多视角视频数据稀缺导致的过拟合问题。
selection_source: fresh_fetch
motivation: 现有的定制化视频生成方法多将物体视为2D实体，缺乏重建3D几何所需的空间先验，导致生成的新视角视频缺乏一致性。
method: 提出3DreamBooth框架，通过1帧优化解耦空间与时间，并结合3Dapter模块利用非对称调节策略从少量参考图中提取视角特定的几何提示。
result: 该方法能够生成具有高保真纹理和强3D一致性的动态视频，且无需大规模多视角视频数据集进行训练。
conclusion: 3DreamBooth为3D感知的视频定制化提供了一种高效方案，通过解耦几何与运动显著提升了生成视频的质量和视角稳定性。
---
