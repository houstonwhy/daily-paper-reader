...
## 摘要
交互式世界模型通过响应用户的动作持续生成视频，从而实现了开放式生成能力。然而，现有模型通常缺乏环境的 3D 表示，这意味着 3D 一致性必须从数据中隐式学习，且空间记忆被限制在有限的时间上下文窗口内。这导致了不真实的用户体验，并为智能体训练等下游任务带来了重大障碍。为了解决这一问题，我们提出了 PERSIST，这是一种新型的世界模型范式，它模拟了潜空间 3D 场景（包括环境、相机和渲染器）的演变。这使我们能够合成具有持久空间记忆和一致几何结构的的新帧。定量指标和定性用户研究均表明，与现有方法相比，该方法在空间记忆、3D 一致性和长时稳定性方面有显著提升，从而能够生成连贯且不断演化的 3D 世界。我们进一步展示了新的功能，包括从单张图像合成多样的 3D 环境，以及通过支持直接在 3D 空间中进行环境编辑和规范，实现对生成体验的细粒度、几何感知的控制。项目页面：https://francelico.github.io/persist.github.io
