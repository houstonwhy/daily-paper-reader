...
tags: ["query:3dgen"]
score: 8.0
evidence: 具有空间保真度的图像到3D多实例生成
tldr: 本研究针对图像到3D多实例生成中的空间保真度问题，提出了TIMI框架。针对现有方法微调成本高且易出现实例纠缠的缺陷，TIMI无需额外训练，通过利用预训练模型中的空间先验，引入实例感知分离引导（ISG）和空间稳定几何自适应更新（SGU）模块，实现了高保真度的多实例3D生成，在保持全局布局的同时显著提升了局部实例的生成质量。
selection_source: fresh_fetch
motivation: 现有的多实例3D生成方法依赖高成本微调且难以保证空间保真度，而预训练模型中蕴含的空间先验未被充分利用。
method: 提出了包含实例感知分离引导（ISG）和空间稳定几何自适应更新（SGU）的无需训练框架，用于在去噪阶段解耦实例并稳定其几何特征。
result: 实验表明，该方法在全局布局和局部实例质量上均优于现有方法，且无需额外训练，推理速度更快。
conclusion: TIMI证明了通过有效利用预训练模型的空间先验，可以在无需训练的情况下实现高质量、高保真度的多实例图像到3D生成。
---
