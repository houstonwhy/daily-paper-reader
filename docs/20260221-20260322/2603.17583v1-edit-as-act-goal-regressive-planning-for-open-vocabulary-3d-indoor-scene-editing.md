...
## 摘要
基于自然语言编辑 3D 室内场景在概念上虽然直观，但在技术上极具挑战性。现有的开放词汇系统通常会重新生成场景的大部分内容，或者依赖于破坏空间结构的图像空间编辑，从而导致意外的全局变化或物理上不一致的布局。这些局限性源于将编辑主要视为一项生成任务。我们提出了不同的观点：用户指令定义了一个期望的世界状态，而编辑应该是使该状态成真并同时保留其他所有内容的最小动作序列。这一视角促成了 Edit-As-Act，这是一个将开放词汇场景编辑视为 3D 空间中目标回归规划的框架。给定源场景和自由形式的指令，Edit-As-Act 预测符号化目标谓词，并在 EditLang 中进行规划。EditLang 是我们设计的一种受 PDDL 启发的动作语言，具有编码支撑、接触、碰撞和其他几何关系的显式前提条件和效果。语言驱动的规划器提出动作，验证器强制执行目标导向性、单调性和物理可行性，从而产生可解释且物理连贯的变换。通过将推理与底层生成分离，Edit-As-Act 实现了指令忠实度、语义一致性和物理合理性——这是现有范式无法同时满足的三个标准。在包含 9 个室内环境、63 个编辑任务的基准测试 E2A-Bench 上，Edit-As-Act 在所有编辑类型和场景类别中均显著优于之前的方法。
