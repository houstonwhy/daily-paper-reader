...
motivation: 现有的动态高斯泼溅方法多依赖逐帧优化，容易导致对瞬时状态的过拟合且缺乏长期的运动连贯性。
method: 提出一种基于时间Transformer和稀疏控制节点的多帧引导框架，利用解耦的规范位置和潜码来稳定运动传播并防止漂移。
result: 实验表明Mango-GS在重建质量上达到了最先进水平，并能支持高保真动态场景的实时交互式渲染。
conclusion: 该研究证明了结合多帧时序建模与节点引导机制能有效增强动态场景重建的时空一致性与鲁棒性。
---
