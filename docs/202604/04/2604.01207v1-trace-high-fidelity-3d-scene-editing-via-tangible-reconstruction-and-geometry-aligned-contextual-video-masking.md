...
## 摘要
我们提出了 TRACE，一个由网格（mesh）引导的 3DGS 编辑框架，实现了自动化的、高保真的场景变换。通过将视频扩散模型锚定在显式 3D 几何上，TRACE 独特地实现了细粒度的局部操作（例如局部姿态偏移或组件替换），同时保持了核心主体的结构完整性，而这一能力在现有编辑方法中很大程度上是缺失的。我们的方法包含三个关键阶段：（1）多视图 3D 锚点合成，利用在 MV-TRACE 数据集上训练的稀疏视图编辑器来生成空间一致的 3D 锚点，该数据集是首个专门用于场景一致性物体添加与修改的多视图一致性数据集；（2）实体几何锚定（TGA），通过两阶段配准确保插入的网格与 3DGS 场景之间的精确空间同步；（3）上下文视频掩码（CVM），将 3D 投影集成到自回归视频流水线中，以实现时间稳定且符合物理规律的渲染。大量实验表明，TRACE 在编辑通用性和结构完整性方面始终优于现有方法。
