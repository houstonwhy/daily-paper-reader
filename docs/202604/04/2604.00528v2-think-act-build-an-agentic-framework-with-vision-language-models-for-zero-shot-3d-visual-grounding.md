...
method: 构建了一个动态智能体框架，通过VLM调用视觉工具在2D帧中追踪目标，并利用几何扩展机制将多视角特征聚合为3D坐标。
result: 在ScanRefer和Nr3D数据集上，该框架凭借开源模型实现了领先的零样本性能，并成功超越了多个全监督基线模型。
conclusion: 通过解耦空间语义理解与几何结构实例化，TAB框架为实现高效、精准的零样本3D视觉定位提供了新的范式。
---
