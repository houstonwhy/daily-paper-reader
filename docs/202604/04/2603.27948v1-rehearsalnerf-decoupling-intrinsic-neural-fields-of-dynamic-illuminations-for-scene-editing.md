...
method: 利用稳定光照下的预采集场景建立几何一致性，并引入可学习的光照向量与基于光流的正则化来实现光影解耦。
result: 在动态光照条件下实现了高质量的新视角合成，并展示了出色的场景编辑和动态物体处理能力。
conclusion: RehearsalNeRF 通过引入“排练”阶段的先验知识，成功解决了动态光照场景下的神经场解耦与编辑难题。
---

## 摘要
尽管神经辐射场（Neural Radiance Fields）已取得显著进展，但动态照明变化的问题仍未得到解决。与对场景中时变/时不变分量进行参数化的相关工作不同，主体的辐射度在时空域中与其自身的发射辐射度和照明颜色高度耦合。在本文中，我们提出了一种名为 RehearsalNeRF 的新型有效方法，用于在剧烈照明变化下学习解耦的神经场。我们的核心思想是利用在稳定光照下拍摄的场景（如排练阶段，在动态照明发生前易于获取），来强制不同光照条件之间的几何一致性。具体而言，RehearsalNeRF 采用了一个可学习的光效向量，该向量在时间维度上表示照明颜色，并用于将投影光颜色从场景辐射度中解耦。此外，我们的 RehearsalNeRF 还能通过简单地采用现成的交互式掩码来重建动态物体的神经场。为了解耦动态物体，我们提出了一种利用光流的新型正则化方法，为颜色解耦提供粗略监督。我们通过在动态照明条件下新视角合成和场景编辑方面的鲁棒性能，证明了 RehearsalNeRF 的有效性。我们的源代码和视频数据集将公开。
