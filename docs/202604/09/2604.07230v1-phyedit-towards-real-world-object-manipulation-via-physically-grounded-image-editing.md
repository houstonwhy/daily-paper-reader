...
tldr: 针对现有图像编辑模型在物体操控中缺乏物理准确性和空间一致性的问题，本文提出了 PhyEdit 框架。该方法通过引入显式几何模拟作为 3D 感知视觉引导，并结合 2D-3D 联合监督，显著提升了物体缩放和定位的精确度。此外，研究者发布了 RealManip-10K 数据集和 ManipEval 评测基准，实验证明 PhyEdit 在 3D 几何准确性和操控一致性上优于现有模型。
selection_source: fresh_fetch
motivation: 现有生成模型因缺乏 3D 几何和透视投影机制，导致在图像编辑中难以实现精确的物体空间操控（如缩放和定位）。
method: 提出 PhyEdit 框架，利用显式几何模拟作为即插即用的 3D 先验，并结合 2D-3D 联合监督来增强物理感知能力。
result: 实验表明，该方法在 3D 几何精度和操控一致性方面显著优于现有开源及闭源模型。
conclusion: 通过将物理几何先验融入生成模型，可以有效解决图像编辑中的空间失真问题，提升了交互式世界模型的物理真实性。
---

## 摘要
在图像编辑中实现符合物理规律的准确物体操控，对于其在交互式世界模型中的潜在应用至关重要。然而，现有的视觉生成模型往往无法进行精确的空间操控，导致物体的缩放和定位不准确。这一局限性主要源于缺乏结合 3D 几何和透视投影的显式机制。为了实现精确的操控，我们开发了 PhyEdit，这是一个利用显式几何模拟作为上下文 3D 感知视觉引导的图像编辑框架。通过将这种即插即用的 3D 先验与 2D-3D 联合监督相结合，我们的方法有效地提高了物理准确性和操控一致性。为了支持该方法并评估性能，我们提出了一个真实世界数据集 RealManip-10K，用于 3D 感知物体操控，该数据集包含成对的图像和深度标注。我们还提出了 ManipEval，这是一个具有多维指标的基准测试，用于评估 3D 空间控制和几何一致性。大量实验表明，我们的方法在 3D 几何准确性和操控一致性方面均优于现有方法，包括强大的闭源模型。
