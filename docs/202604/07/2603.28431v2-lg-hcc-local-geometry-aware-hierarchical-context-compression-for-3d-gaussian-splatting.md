...
tags: ["query:3dgen"]
score: 7.0
evidence: 3D高斯泼溅表示的压缩框架
tldr: 针对3D高斯泼溅（3DGS）存储开销巨大的挑战，本文提出LG-HCC压缩框架。该框架通过引入局部几何感知机制，在锚点剪枝阶段利用邻域特征聚合保留结构完整性，并结合几何引导卷积实现分层熵编码。LG-HCC在大幅降低存储需求的同时，有效解决了现有方法中的结构退化问题，在保持高保真渲染质量的前提下，实现了优于当前主流方案的率失真性能。
selection_source: fresh_fetch
motivation: 现有的锚点式3DGS压缩方案因忽略显式的几何依赖关系，导致压缩过程中出现结构退化和率失真性能不佳的问题。
method: 提出邻域感知锚点剪枝策略以优化锚点结构，并结合几何引导卷积算子构建分层熵编码方案，实现空间自适应的上下文建模。
result: 实验证明LG-HCC在维持优异几何完整性和渲染保真度的同时，其压缩效率和视觉质量均超越了现有的最先进锚点压缩方法。
conclusion: LG-HCC通过整合局部几何相关性，有效解决了3DGS压缩中的结构保持瓶颈，为高效、紧凑的3D场景表示提供了新思路。
---

## 摘要
尽管 3D 高斯泼溅（3DGS）能够实现高保真实时渲染，但其巨大的存储开销严重阻碍了实际部署。最近基于锚点（anchor-based）的 3DGS 压缩方案通过一些先进的上下文模型减少了高斯冗余。然而，这些方法忽略了显式的几何依赖关系，导致结构退化和次优的率失真性能。在本文中，我们提出了 LG-HCC，这是一种几何感知的 3DGS 压缩框架，它将锚点间的几何相关性纳入锚点剪枝和熵编码中，以实现紧凑表示。具体而言，我们引入了一种邻域感知锚点剪枝（NAAP）策略，该策略通过加权邻域特征聚合评估锚点重要性，并将冗余锚点合并到显著的邻居中，从而产生一个紧凑且几何一致的锚点集。基于这一优化结构，我们进一步开发了一种分层熵编码方案，其中通过轻量级的几何引导卷积（GG-Conv）算子利用从粗到细的先验，以实现空间自适应上下文建模和率失真优化。大量实验表明，LG-HCC 有效地解决了结构保持瓶颈，与最先进的基于锚点的压缩方法相比，保持了卓越的几何完整性和渲染保真度。
