...
tldr: "3D Gaussian Splatting (3DGS) 虽在实时渲染表现出色，但海量高斯点导致内存开销巨大。本文提出 GS^2，通过图论优化空间分布。核心贡献包括基于 ELBO 的自适应加密策略、不透明度感知渐进剪枝以及图特征编码引导的点位移模块。该方法在仅使用 12.5% 高斯点的情况下，实现了比原始 3DGS 更高的渲染质量，兼顾了紧凑性与视觉效果。"
selection_source: fresh_fetch
motivation: 针对现有 3DGS 剪枝方法容易破坏空间一致性并产生渲染伪影的问题，寻求一种在减少内存占用的同时优化空间分布的方案。
method: 引入了基于证据下界（ELBO）的自适应加密、不透明度感知渐进剪枝以及基于图特征编码的空间分布调整模块。
result: "在仅保留约 12.5% 高斯点的情况下，GS^2 实现了比原始 3DGS 更高的 PSNR，并在渲染质量和内存效率上优于现有基准。"
conclusion: GS^2 通过优化高斯点的空间分布，成功实现了高效、紧凑且高质量的 3D 场景重建。
---
