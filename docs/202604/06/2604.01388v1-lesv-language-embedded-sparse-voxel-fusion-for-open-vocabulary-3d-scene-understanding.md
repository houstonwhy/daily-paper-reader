...
method: 利用稀疏体素光栅化构建结构化几何基础，并结合单目先验与AM-RADIO模型的密集对齐特征进行融合。
result: 在开放词汇3D物体检索和点云理解基准测试中取得SOTA性能，特别是在细粒度查询方面表现优异。
conclusion: 该研究证明了结构化体素表示和密集对齐特征在解决3D场景理解中语义漂移和歧义问题的有效性。
---

## 摘要
开放词汇 3D 场景理解的最新进展在很大程度上依赖于 3D 高斯泼溅 (3DGS) 将视觉语言特征配准到 3D 空间中。然而，我们发现了这些方法中的两个关键局限性：一是由于非结构化、重叠的高斯分布导致的物理空间歧义，这使得概率特征配准成为必要；二是由于在物体级掩码上池化特征而导致的多级语义歧义，这稀释了细粒度的细节。为了应对这些挑战，我们提出了一个新颖的框架，利用稀疏体素光栅化 (SVRaster) 作为一种结构化的、不相交的几何表示。通过使用单目深度和法向先验对 SVRaster 进行正则化，我们建立了一个稳定的几何基础。这实现了一个确定性的、置信度感知的特征配准过程，并抑制了 3DGS 中常见的语义混叠现象。此外，我们利用基础模型 AM-RADIO 新兴的密集对齐特性来解决多级歧义，从而避免了分层训练方法的计算开销。我们的方法在开放词汇 3D 物体检索和点云理解基准测试中达到了最先进的性能，特别是在配准方法通常失效的细粒度查询上表现出色。
