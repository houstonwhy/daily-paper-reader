...
tags: ["query:3dgen"]
score: 7.0
evidence: 基于鱼眼相机输入的3D场景重建
tldr: 针对3D高斯泼溅（3DGS）处理鱼眼图像时需去畸变导致的边缘信息丢失和细节模糊问题，本文提出DirectFisheye-GS。该方法将鱼眼模型直接集成到3DGS框架中，支持原生图像输入，并引入特征重叠驱动的跨视图联合优化策略，以解决边缘畸变引起的伪影。实验证明，该方法在保持大视场优势的同时，显著提升了重建质量，性能达到或超越了现有SOTA水平。
selection_source: fresh_fetch
motivation: 传统的鱼眼图像去畸变预处理会造成边缘信息丢失和重采样导致的细节模糊，限制了3DGS在广角场景下的应用效果。
method: 将鱼眼相机模型直接整合进3DGS渲染流程，并提出一种基于特征重叠的跨视图联合优化策略来增强几何与光度一致性。
result: 在公开数据集上的测试表明，该方法有效消除了边缘浮动伪影，重建质量匹配或超越了当前最先进的技术。
conclusion: DirectFisheye-GS通过原生支持鱼眼输入和跨视图优化，为大视场场景的高质量3D重建提供了一种高效且通用的解决方案。
---

## 摘要
3D 高斯泼溅 (3DGS) 实现了从日常图像中进行高效的 3D 场景重建，并具备实时、高保真的渲染能力，极大地推动了 VR/AR 应用的发展。鱼眼相机凭借其更宽的视场角 (FOV)，有望通过更少的输入实现高质量重建，近期受到了广泛关注。然而，由于 3DGS 依赖于光栅化，大多数涉及鱼眼相机输入的工作在训练前会先对图像进行去畸变处理，这引入了两个问题：1) 图像边缘的黑边导致信息丢失，抵消了鱼眼相机大视场角的优势；2) 去畸变过程中的拉伸与插值重采样将每个像素的值分散到更大的区域，稀释了细节密度，导致 3DGS 在这些低频区域产生过拟合，从而产生模糊和漂浮伪影。在这项工作中，我们将鱼眼相机模型集成到原始 3DGS 框架中，实现了无需预处理的原生鱼眼图像输入训练。尽管建模正确，我们观察到重建场景在图像边缘仍会出现漂浮物：畸变随边缘增加，且 3DGS 原始的逐迭代随机视图选择优化策略忽略了高斯球的跨视图相关性，导致产生极端形状（如过大或过长），从而降低了重建质量。为解决这一问题，我们引入了一种特征重叠驱动的跨视图联合优化策略，在不同视图间建立了一致的几何和光度约束——该技术同样适用于现有的基于针孔相机的流水线。我们的 DirectFisheye-GS 在公开数据集上达到或超过了现有最先进方法的性能。
