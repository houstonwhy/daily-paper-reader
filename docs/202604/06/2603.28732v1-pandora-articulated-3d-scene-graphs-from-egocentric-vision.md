...
tags: ["query:3dgen"]
score: 7.0
evidence: 从第一人称视角构建度量语义场景表示
tldr: 针对机器人自主建图受限于自身运动能力（如无法打开抽屉）导致环境信息缺失的问题，本文提出Pandora系统。该系统利用人类佩戴Project Aria眼镜采集的第一视角数据，通过简单启发式方法提取物体关节模型，并构建包含关节信息的3D场景图。实验证明，该方法能有效提升机器人对环境动态及容器关系的理解，并成功引导Spot机器人在移动操作任务中寻找隐藏目标。
selection_source: fresh_fetch
motivation: 传统机器人建图受限于机器人自身的物理局限，难以探索抽屉、橱柜等具有关节结构的封闭空间。
method: 利用人类自然探索环境时的第一视角视频数据，通过启发式算法恢复物体的关节模型，并将其整合进3D场景图中。
result: 该方法生成的关节模型质量可媲美现有先进技术，并能显著增强机器人对物体动态和容器关系的认知。
conclusion: 结合人类视角的关节化3D场景图能有效弥补机器人感知盲区，显著提升其在复杂环境下的移动操作任务表现。
---
