...
tldr: 本文探讨了动态3D高斯泼溅（3DGS）技术在第一人称视角（Egocentric）场景重建中的应用。针对第一人称视频中相机运动剧烈和场景动态复杂的挑战，研究者利用EgoExo4D数据集中的配对视角，评估了现有单目动态3DGS模型的表现。研究发现，第一人称视角的重建质量普遍低于第三人称视角，且性能差距主要源于静态背景的重建。这一发现揭示了现有模型的局限性，并为开发针对第一人称视角的特定重建算法提供了动力。
selection_source: fresh_fetch
motivation: 旨在探究现有的动态3D高斯泼溅模型是否能有效应对第一人称视角视频中剧烈的相机运动和复杂的场景动态。
method: 通过在EgoExo4D数据集的配对第一人称与第三人称视频上运行单目动态3DGS模型，进行定量的对比评估。
result: 实验表明第一人称视角的重建质量（PSNR）明显较低，且这种性能下降主要归因于静态内容的重建失败而非动态部分。
conclusion: 研究强调了开发针对第一人称视角特定解决方案的必要性，并提倡在评估中区分视频的静态和动态区域。
---

## 摘要
第一视角（Egocentric）视频为人类感知和交互提供了独特的视角，在增强现实、机器人技术和辅助技术领域的重要性日益凸显。然而，快速的相机运动和复杂的场景动态为该视角下的 3D 重建带来了重大挑战。尽管 3D 高斯泼溅（3DGS）已成为高效、高质量新视角合成的先进方法，但专注于从单目视频重建动态场景的变体很少在第一视角视频上进行评估。目前尚不清楚现有模型是否能泛化到这一场景，或者是否需要专门针对第一视角的解决方案。在这项工作中，我们利用 EgoExo4D 数据集中的第一视角-第三视角（ego-exo）配对录像，评估了动态单目 3DGS 模型在第一视角和第三视角视频上的表现。我们发现，第一视角的重建质量始终较低。分析表明，以峰值信噪比（PSNR）衡量的重建质量差异源于静态内容而非动态内容的重建。我们的研究结果强调了当前的局限性，并推动了针对第一视角特定方法的发展，同时也突显了分别评估视频中静态和动态区域的价值。
