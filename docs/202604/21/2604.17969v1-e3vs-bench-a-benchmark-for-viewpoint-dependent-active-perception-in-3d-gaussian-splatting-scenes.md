...
title_zh: E3VS-Bench：3D Gaussian Splatting 场景中视点依赖型主动感知的基准测试
...
conclusion: E3VS-Bench证明了当前模型在复杂3D环境中的主动感知瓶颈，为未来具身智能的视角规划研究提供了重要参考。
---

## 摘要
3D 环境中的视觉搜索要求具身智能体主动探索周围环境并获取与任务相关的证据。然而，现有的视觉搜索和具身智能（Embodied AI）基准测试（包括 EQA）通常依赖于静态观测或受限的第一人称运动，因此未能明确评估在真实 3D 环境中不受限的 5 自由度（5-DoF）视点控制下出现的细粒度视点依赖现象，例如由垂直视点偏移引起的可见性变化、揭示容器内部物品，以及消除仅能从特定角度观察到的物体属性歧义。为了解决这一局限性，我们推出了 E3VS-Bench，这是一个具身 3D 视觉搜索基准测试，要求智能体在 5 自由度下控制其视点，以收集用于问答的视点依赖型证据。E3VS-Bench 包含 99 个使用 3D Gaussian Splatting 重建的高保真 3D 场景和 2,014 个问题驱动的情节。3D Gaussian Splatting 实现了照片级真实感的自由视点渲染，保留了在基于网格（mesh-based）的模拟器中经常退化的细粒度视觉细节（如微小文字和微妙属性），从而允许构建无法从单一视角回答、而需要跨 5 自由度视点进行主动检查的问题。我们评估了多个最先进的视觉语言模型（VLM），并将其性能与人类进行了比较。尽管具有强大的 2D 推理能力，所有模型与人类之间仍存在巨大差距，这凸显了在全 5 自由度视点变化下，模型在主动感知和连贯视点规划方面的局限性。
