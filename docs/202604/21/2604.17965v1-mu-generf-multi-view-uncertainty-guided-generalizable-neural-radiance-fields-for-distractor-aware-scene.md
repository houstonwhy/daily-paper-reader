...
tldr: 针对通用神经辐射场（GeNeRF）在处理包含瞬时干扰物场景时鲁棒性不足的问题，本文提出了MU-GeNeRF框架。该方法引入多视图不确定性引导机制，将不确定性分解为捕捉源视图结构差异的源视图不确定性和检测目标图像观测异常的目标视图不确定性。通过异方差重建损失，模型能自适应调节监督强度，有效抑制干扰物并提升几何建模质量，在泛化性能和抗干扰能力上均表现优异。
selection_source: fresh_fetch
motivation: 现有的通用神经辐射场在面对破坏跨视图一致性的瞬时干扰物时，难以区分静态结构与动态干扰，导致重建质量严重下降。
method: 提出一种多视图不确定性引导框架，通过解耦源视图与目标视图的不确定性，并利用异方差重建损失实现自适应的干扰物抑制与鲁棒建模。
result: 实验证明该方法在干扰物场景下的表现显著优于现有通用NeRF，并达到了与场景特定去干扰方法相媲美的性能。
conclusion: MU-GeNeRF通过创新的多视图不确定性建模，有效提升了泛化神经辐射场在复杂动态环境下的鲁棒性和重建精度。
---

## 摘要
可泛化神经辐射场 (GeNeRFs) 能够从稀疏视图中实现高质量的场景重建，并可泛化到未见场景。然而，在现实世界设置中，瞬态干扰物会破坏跨视图的结构一致性，从而损害监督信号并降低重建质量。现有的无干扰物 NeRF 方法依赖于逐场景优化，并根据逐视图重建误差估计不确定性，这对于 GeNeRF 而言并不可靠，且经常将不一致的静态结构误判为干扰物。为此，我们提出了 MU-GeNeRF，这是一个多视图不确定性引导的干扰物感知 GeNeRF 框架，旨在缓解 GeNeRF 在存在瞬态干扰时的鲁棒建模挑战。我们将干扰物感知分解为两个互补的不确定性组件：源视图不确定性 (Source-view Uncertainty)，用于捕捉由视角变化或动态因素引起的源视图间的结构差异；以及目标视图不确定性 (Target-view Uncertainty)，用于检测由瞬态干扰物引起的目标图像观测异常。这两种不确定性针对不同的误差来源，并通过异方差重建损失进行结合，引导模型自适应地调节监督信号，从而实现更鲁棒的干扰物抑制和几何建模。大量实验表明，我们的方法不仅超越了现有的 GeNeRF，而且达到了与特定场景的无干扰物 NeRF 相当的性能。
