...
tldr: 本研究提出Ψ-Map框架，旨在解决大规模场景下3D高斯泼溅在几何精度、全景理解和实时性方面的挑战。通过结合LiDAR约束的2D高斯面元表示、端到端查询引导的全景学习架构以及优化的渲染策略，该系统实现了高精度的表面重建和全局一致的语义理解。实验证明，该方法在保持超过40 FPS实时推理速度的同时，显著提升了重建质量，为机器人Real2Sim迁移提供了强有力的感知支持。
selection_source: fresh_fetch
motivation: 现有3DGS方法在大规模场景中难以同时满足几何精度、全景一致性以及实时推理的需求。
method: 采用LiDAR约束的2D高斯面元强化几何，并利用查询引导的端到端架构与Top-K硬选择策略实现高效的全景特征融合与渲染。
result: 在保证大规模场景重建质量领先的同时，推理速度突破40 FPS，达到了机器人控制回路的实时性标准。
conclusion: Ψ-Map通过集成几何强化与高效全景学习，为实现高精度的实时Real2Sim转换和机器人感知提供了可靠的技术路径。
---
