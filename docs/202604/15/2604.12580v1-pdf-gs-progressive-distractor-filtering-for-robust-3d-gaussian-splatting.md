...
tldr: 针对3D高斯泼溅（3DGS）在处理多视图不一致（如动态干扰物）时易产生伪影的问题，本文提出PDF-GS框架。该方法挖掘并放大3DGS内在的信号抑制能力，通过渐进式多阶段优化，利用差异线索逐步过滤干扰物并恢复视图一致的精细细节。PDF-GS无需修改架构且无推理开销，在多个挑战性数据集上实现了鲁棒的高保真重建，达到了当前领先水平。
selection_source: fresh_fetch
motivation: 传统的3DGS训练假设多视图完全一致，导致其在面对现实场景中的动态干扰物时容易产生严重的视觉伪影。
method: 提出一种渐进式干扰过滤框架，通过多阶段优化利用差异线索剔除不一致信号，并随后通过重建阶段恢复一致的场景细节。
result: 在多种复杂现实场景下，PDF-GS在保持轻量化和零额外推理开销的同时，重建质量显著优于现有基准方法。
conclusion: PDF-GS通过强化3DGS的自过滤特性，为实现鲁棒、高保真且无干扰的3D场景重建提供了一种高效且易于适配的通用方案。
---
