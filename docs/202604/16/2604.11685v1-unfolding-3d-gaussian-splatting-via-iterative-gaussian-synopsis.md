...
tldr: 针对3DGS存储开销大及现有LOD方案冗余的问题，本文提出“迭代高斯综述”框架。该方法采用自上而下的“展开”方案，通过可学习掩码剪枝从全分辨率模型迭代构建多级层次结构。结合分层空间网格与共享锚点码本，实现了紧凑的特征表示并支持渐进式渲染。实验证明，该方法在显著降低存储的同时保持了高渲染质量，提升了3DGS在资源受限环境下的实用性。
selection_source: fresh_fetch
motivation: 解决3DGS存储需求过大及现有LOD方案在资源受限环境下存在的冗余和保真度下降问题。
method: 提出一种自上而下的展开机制，利用掩码剪枝构建多级LOD，并结合分层空间网格与共享锚点码本进行特征压缩。
result: 在大幅减少存储空间的同时，实现了各层级的高质量渲染，并支持极低数据开销的渐进式细化。
conclusion: 该研究为带宽和内存受限场景下的实时3DGS部署提供了一种高效、可扩展且紧凑的表示框架。
---

## 摘要
3D 高斯泼溅 (3DGS) 已成为实时、高保真新视角合成的最先进框架。然而，其巨大的存储需求和固有的非结构化表示为在流媒体和资源受限环境中的部署带来了挑战。现有的细节层次 (LOD) 策略，特别是那些基于自下而上构建的策略，往往会引入冗余或导致保真度下降。为了克服这些局限性，我们提出了迭代高斯摘要 (Iterative Gaussian Synopsis)，这是一个通过自上而下的“展开”方案实现紧凑且渐进式渲染的新型框架。我们的方法从全分辨率 3DGS 模型开始，并使用一种自适应、可学习的基于掩码的剪枝机制，迭代地推导出更粗糙的 LOD。该过程构建了一个多级层次结构，在提高效率的同时保持了视觉质量。我们将捕获全局场景结构的层次化空间网格与建模局部细节的共享锚点码本 (Anchor Codebook) 相结合。这种结合产生了一种紧凑且具有表现力的特征表示，旨在最大限度地减少冗余并支持高效的、针对特定层级的自适应。展开机制促进了层间可重用性，并且在渐进式细化过程中仅需极小的数据开销。实验表明，我们的方法在所有 LOD 级别上均保持了高渲染质量，同时实现了大幅度的存储缩减。这些结果证明了我们的方法在带宽和内存受限场景下进行实时 3DGS 渲染的实用性和可扩展性。
