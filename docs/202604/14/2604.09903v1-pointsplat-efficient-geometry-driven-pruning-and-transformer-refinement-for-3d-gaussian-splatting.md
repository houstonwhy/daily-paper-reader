...
motivation: 传统的3DGS剪枝方法通常依赖2D图像计算重要性得分且需要耗时的逐场景微调，限制了其在资源受限环境下的应用。
...
conclusion: PointSplat通过几何驱动的剪枝与特征细化，为实现轻量化且高效的3D场景重建提供了一种新颖且无需微调的解决方案。
---

## 摘要
3D 高斯泼溅 (3DGS) 最近通过使用显式 3D 原语表示场景，实现了实时、高保真的新视角合成。然而，传统方法通常需要数百万个高斯分布来捕捉复杂场景，导致了巨大的内存和存储需求。最近的方法通过对高斯参数进行剪枝和逐场景微调解决了这一问题，从而在保持视觉质量的同时减小了模型大小。这些策略通常依赖 2D 图像来计算重要性得分，随后进行特定场景的优化。在这项工作中，我们介绍了 PointSplat，这是一个 3D 几何驱动的剪枝与细化框架，它桥接了此前互不相关的两个研究方向：高斯剪枝和 Transformer 细化。我们的方法包含两个关键组件：(1) 一种高效的几何驱动策略，仅根据高斯的 3D 属性对其进行排序，从而在剪枝阶段消除了对 2D 图像的依赖；(2) 一个双分支编码器，对几何和外观特征进行分离和重加权，以避免特征不平衡。在 ScanNet++ 和 Replica 数据集上针对不同稀疏度水平进行的广泛实验表明，PointSplat 在无需额外逐场景优化的前提下，始终能实现具有竞争力的渲染质量和卓越的效率。
