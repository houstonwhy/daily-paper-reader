...
tldr: 针对3D高斯泼溅（3DGS）在处理多视图不一致干扰物（如移动物体、遮挡）时易产生伪影的问题，本文提出了PDF-GS框架。该方法利用3DGS固有的信号抑制能力，通过渐进式多阶段优化策略，利用差异线索逐步过滤干扰物，并恢复一致的精细细节。PDF-GS在保持轻量级且无需改变推理架构的前提下，显著提升了复杂场景下的重建质量，达到了新的SOTA水平。
selection_source: fresh_fetch
motivation: 传统的3DGS训练假设输入图像具有完全的多视图一致性，导致其在面对干扰物时容易产生视觉伪影。
method: 提出一种渐进式干扰物过滤框架，通过多阶段优化利用差异线索剔除不一致信号，并随后进行精细化重建。
result: 在多个数据集和挑战性现实场景中，PDF-GS一致优于现有基准方法，实现了高保真且无干扰的重建效果。
conclusion: PDF-GS是一种轻量且易于适配的鲁棒3DGS方案，在不增加推理开销的情况下显著提升了复杂环境下的渲染性能。
---

## 摘要
近期 3D 高斯泼溅（3DGS）的进展实现了令人印象深刻的实时照片级真实感渲染。然而，传统的训练流程本质上假设输入图像之间具有完全的多视图一致性，这使得它们对违反该假设的干扰物（distractors）非常敏感，从而导致视觉伪影。在这项工作中，我们重新审视了 3DGS 一个尚未被充分探索的特性：其抑制不一致信号的固有能力。基于这一见解，我们提出了 PDF-GS（面向鲁棒 3D 高斯泼溅的渐进式干扰物过滤），这是一个通过渐进式多阶段优化来增强这种自过滤特性的框架。渐进式过滤阶段通过利用差异线索逐渐移除干扰物，而随后的重建阶段则从纯化后的高斯表示中恢复细粒度的、视图一致的细节。通过这种迭代优化，PDF-GS 实现了鲁棒、高保真且无干扰物的重建，在各种数据集和具有挑战性的现实场景中始终优于基准方法。此外，我们的方法是轻量级的，且易于适配现有的 3DGS 框架，无需更改架构或增加额外的推理开销，从而达到了新的最先进性能。代码已在 https://github.com/kangrnin/PDF-GS 公开。
