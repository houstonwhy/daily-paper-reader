...
tldr: 针对语言驱动的3D高斯泼溅（3DGS）编辑中2D编辑证据与3D高斯语义对应关系不明确的问题，本文提出了TransSplat。该方法将编辑过程建模为多视图非平衡语义传输问题，通过建立可见高斯与视图特定编辑原型之间的对应关系，恢复共享的规范3D编辑场，并利用传输残差抑制非目标区域的错误编辑，显著提升了局部编辑的准确性和结构一致性。
selection_source: fresh_fetch
motivation: 现有3DGS编辑方法主要关注视图一致性，却忽视了2D编辑证据与3D高斯之间根本的语义对应关系。
method: 将编辑建模为多视图非平衡语义传输问题，通过建立高斯与编辑原型的对应关系来指导3D外观更新，并利用传输残差减少编辑泄漏。
result: 实验结果表明，TransSplat在局部编辑精度和结构一致性方面均优于现有的以增强视图一致性为核心的3D编辑方法。
conclusion: 该研究通过显式建模语义对应关系，为语言驱动的3D场景编辑提供了一种更精确、更具局部控制能力的解决方案。
---

## 摘要
语言驱动的 3D 高斯泼溅 (3DGS) 编辑为 VR/AR 中复杂场景的修改提供了一种更便捷的方法。标准流水线通常采用两阶段策略：首先编辑多个 2D 视图，然后优化 3D 表示以匹配这些编辑后的观测结果。现有方法主要通过多视图特征融合、注意力过滤或迭代重新校准来提高视图一致性。然而，它们未能明确解决一个更根本的问题：编辑后的 2D 证据与 3D 高斯之间的语义对应关系。为了解决这一问题，我们提出了 TransSplat，它将语言驱动的 3DGS 编辑建模为一个多视图不平衡语义传输问题。具体而言，我们的方法建立了可见高斯与特定视图编辑原型之间的对应关系，从而明确刻画了编辑后的 2D 证据与 3D 高斯之间的语义关系。它进一步恢复了一个跨视图共享的规范 3D 编辑场，以指导统一的 3D 外观更新。此外，我们利用传输残差来抑制非目标区域的错误编辑，从而减轻编辑泄漏并提高局部控制精度。定性和定量结果表明，与现有的以增强视图一致性为核心的 3D 编辑方法相比，TransSplat 在局部编辑准确性和结构一致性方面取得了更优越的性能。
