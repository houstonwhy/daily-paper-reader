...
tags: ["query:3dgen"]
score: 7.0
evidence: 3D重建中的语义场景分解
tldr: 针对3D高斯泼溅等重建方法在交互和语义分割中存在的跨视图不一致及质量问题，本文提出Semantic Foam。该方法基于Radiant Foam的体积Voronoi网格结构，在单元级别引入显式语义特征场。通过利用Voronoi单元的固有空间结构进行正则化，有效解决了遮挡和监督不一致带来的伪影，实现了优于Gaussian Grouping等现有方法的物体级分割性能，为交互式图形应用提供了更可靠的场景表示。
selection_source: fresh_fetch
motivation: 现有的3D场景重建方法在语义分解时面临分割质量差和跨视图不一致的挑战，限制了其在交互式应用中的使用。
method: 该方法扩展了Radiant Foam表示，通过在体积Voronoi网格的单元级别定义显式语义特征场来实现空间正则化。
result: 实验结果显示，Semantic Foam在物体级分割性能上显著优于Gaussian Grouping和SAGA等当前最先进的方法。
conclusion: Semantic Foam通过统一空间与语义分解，有效提升了场景重建的语义一致性，增强了三维资产的可交互性。
---
