...
score: 7.0
evidence: 动态场景重建与编码
tldr: 针对实时沉浸式视频通信中多视图编码与3D重建解耦导致的效率低下问题，本文提出GS-SCNet框架。该框架首次将可泛化的3D高斯泼溅（3DGS）与深度语义编码集成，通过视差引导的并行语义编解码器和轻量级高斯参数预测器，实现了跨视图上下文交互并直接从语义潜变量预测3DGS属性。实验证明，该方法在压缩效率、渲染质量和实时性方面均优于传统方案，且具有极强的泛化能力。
selection_source: fresh_fetch
motivation: 传统多视图视频编码与3D重建相互独立，导致传输效率低且计算冗余，难以满足实时沉浸式通信的需求。
...
conclusion: 该研究通过语义编码与3DGS的深度耦合，有效消除了计算瓶颈，为高效、高质量的实时3D远程呈现提供了新方案。
---

## 摘要
实时沉浸式视频通信，特别是高保真 3D 远程呈现，需要在瞬时动态场景重建与高效数据传输之间实现协同平衡。虽然前馈 3D 高斯泼溅（3DGS）的最新进展已实现了实时渲染，但以解耦方式进行多视图视频编码和 3D 重建会导致压缩效率欠佳且计算复杂度较高。为解决这一问题，我们提出了 GS-SCNet，这是首个将可泛化 3DGS 重建与专用深度语义编码流水线无缝集成的统一端到端框架。我们的架构由两项核心技术贡献支撑：(i) 我们引入了一种视差引导的并行语义编解码器，利用对极几何先验，通过视差补偿和语义融合促进跨视图上下文交互，从而在显著提升率失真性能的同时实现立体流的实时并行处理；(ii) 我们开发了一个轻量级高斯参数预测器，直接将解码后的语义潜变量投影为 3DGS 属性，消除了对中间像素域重建的需求。通过将编解码器与特定任务预测器耦合，我们的框架仅需提取一次几何相关性，有效消除了传统解耦范式中固有的冗余计算瓶颈。在合成和真实人类数据集上的广泛评估表明，GS-SCNet 在压缩效率、渲染质量和实时性能之间实现了卓越的权衡。值得注意的是，当应用于域外真实世界数据时，我们的框架表现出强大的跨域泛化能力和对压缩伪影的鲁棒性，显著优于传统的解耦传输范式。
