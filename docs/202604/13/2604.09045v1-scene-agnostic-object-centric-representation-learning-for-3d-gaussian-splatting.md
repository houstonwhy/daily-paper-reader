...
tldr: 针对现有3D场景理解方法依赖2D基础模型掩码导致的多视图一致性差和跨场景泛化难的问题，本文提出一种面向3D高斯泼溅（3DGS）的场景无关对象中心化表示学习方法。通过引入预训练的槽注意力全局对象中心学习（GOCL）模块和对象代码本，实现了无需额外预处理或多视图对齐的无监督对象监督。该方法不仅提升了3DGS的结构化表示能力，还显著增强了其在机器人交互和跨场景任务中的泛化性能。
selection_source: fresh_fetch
motivation: 现有的基于视觉基础模型的3D分割方法存在多视图身份冲突且难以跨场景泛化。
method: 利用预训练的槽注意力机制构建场景无关的对象代码本，直接监督3D高斯的身份特征。
result: 实现了无需每场景微调的无监督对象识别，并提供了更具结构化的3D表示。
conclusion: 将无监督对象中心化学习引入3DGS，有效提升了3D场景理解的泛化能力和下游任务表现。
---
