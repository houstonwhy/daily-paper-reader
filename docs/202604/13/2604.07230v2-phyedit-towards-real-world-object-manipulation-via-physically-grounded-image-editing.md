...
title_zh: "PhyEdit: 通过物理接地图像编辑迈向真实世界物体操控"
...
## 摘要
在图像编辑中实现物理准确的物体操控，对于其在交互式世界模型中的潜在应用至关重要。然而，现有的视觉生成模型往往无法进行精确的空间操控，导致物体的缩放和定位不准确。这一局限性主要源于缺乏整合 3D 几何和透视投影的显式机制。为了实现精确操控，我们开发了 PhyEdit，这是一个利用显式几何模拟作为上下文 3D 感知视觉引导的图像编辑框架。通过将这种即插即用的 3D 先验与 2D-3D 联合监督相结合，我们的方法有效地提高了物理准确性和操控一致性。为了支持该方法并评估性能，我们提出了一个真实世界数据集 RealManip-10K，用于 3D 感知物体操控，该数据集包含成对图像和深度标注。我们还提出了 ManipEval，这是一个具有多维指标的基准测试，用于评估 3D 空间控制和几何一致性。大量实验表明，我们的方法在 3D 几何准确性和操控一致性方面均优于现有方法，包括强大的闭源模型。
