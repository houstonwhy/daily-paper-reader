...
tags: ["query:3dgen"]
score: 6.0
evidence: 用于3D网格关节和姿态空间的生成式视频先验
tldr: ViPS 提出了一种利用预训练视频扩散模型为自动绑定（auto-rigged）的 3D 网格学习有效姿态空间的方法。针对传统骨骼绑定缺乏合理姿态约束、易产生解剖学错误或自相交的问题，该框架通过蒸馏视频先验知识，在无需 4D 艺术家数据的情况下，构建了平滑且可控的姿态分布。它支持多样化采样、逆运动学投影及时间相干的轨迹生成，并在跨物种和拓扑结构上展现出强大的泛化能力。
selection_source: fresh_fetch
motivation: 传统的 3D 骨骼绑定缺乏对合理姿态流形的内在约束，导致随机采样或手动操作时常出现解剖学畸变或物理自相交。
method: 该框架通过将预训练视频扩散模型的运动先验蒸馏到特定骨骼参数分布中，并结合可微分几何验证器来确保姿态的有效性。
result: 仅依靠视频先验训练的 ViPS 在姿态合理性和多样性上达到了与使用合成 4D 数据训练的先进方法相当的水平。
conclusion: ViPS 证明了视频生成模型可作为通用的 3D 运动先验，为不同物种和骨骼拓扑提供了鲁棒的零样本泛化能力。
---

## 摘要
运动学骨架为 3D 网格的关节运动提供了结构化接口，但它们缺乏对给定资产合理关节配置流形的内在表示。如果没有这种姿态空间，对原始骨架参数的随机采样或手动操作往往会导致语义或几何冲突，例如解剖学上的过度伸展和非物理的自相交。我们提出了视频引导姿态空间（ViPS），这是一个前馈框架，通过从预训练的视频扩散模型中蒸馏运动先验，来发现自动绑定网格有效关节运动的潜分布。与依赖稀缺的艺术家创作 4D 数据集的现有方法不同，ViPS 将生成式视频先验转化为给定骨架参数化上的通用分布。应用于蒙皮网格的可微几何验证器在无需手动正则化项的情况下，确保了特定资产的有效性。我们的模型学习了一个平滑、紧凑且可控的姿态空间，支持多样化采样、用于逆运动学的流形投影以及用于关键帧生成的时序相干轨迹。此外，蒸馏出的 3D 姿态样本可作为引导视频扩散的精确语义代理，有效地闭合了生成式 2D 先验与结构化 3D 运动控制之间的环路。我们的评估表明，仅在视频先验上训练的 ViPS 在合理性和多样性方面均达到了在合成艺术家创作 4D 数据上训练的先进方法的性能。最重要的是，作为一个通用模型，ViPS 对分布外物种和未见过的骨骼拓扑结构展现出了鲁棒的零样本泛化能力。
