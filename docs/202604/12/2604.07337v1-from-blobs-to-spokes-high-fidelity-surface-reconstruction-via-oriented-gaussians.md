...
score: 7.0
evidence: 复杂场景的表面重建
tldr: 针对3D Gaussian Splatting（3DGS）在表面重建中缺乏全局几何场的问题，本文提出了Gaussian Wrapping。通过为每个高斯基元引入可学习的定向法线和改进的衰减公式，推导出闭式解的占用场和法线场。结合一致性损失和新型加密策略，该方法能生成完整、水密的网格，在DTU和Tanks and Temples数据集上达到SOTA，并能精准还原自行车辐条等极细微结构。
selection_source: fresh_fetch
motivation: 3DGS基于不透明度的公式缺乏全局几何场，导致现有表面提取方法多依赖启发式策略且难以处理复杂拓扑。
method: 引入定向法线和改进的衰减公式以构建闭式占用场，并利用新型一致性损失和加密策略确保生成完整的表面壳层。
result: 在DTU和Tanks and Temples基准测试中取得领先，能够重建出极细的结构（如自行车辐条）并生成高质量的水密网格。
conclusion: 该研究通过将3DGS转化为连续的占用场模型，克服了传统高斯点云在几何重建上的局限性，显著提升了表面提取的精度和完整性。
---
